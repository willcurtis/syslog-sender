...
Select a row to inspect every parsed field and the original payload. The table follows each newly received event through the active sort and filters, keeping the latest visible log on screen. Use **Expand log view** to hide the listener controls and detail pane temporarily, giving the table more room while retaining filters and capture actions. **Restore layout** brings back the original view and its previous splitter proportions. The **Pause display** control keeps receiving into a bounded queue while holding the visible table steady; resume to add queued messages. **Clear** removes captured data from memory without stopping the listener.
...
Copyright © 2025–2026 The Tech Shed. LogSalvo version 2.6.1.
